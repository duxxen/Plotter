...
namespace Plotter
{
	inline static struct PlotStyle : Style
	{
		...
		enum ShowFlags
		{
			...
			SHOW_VOLUME = SHOW_GRID << 3,
			SHOW_ALL = SHOW_GRID | SHOW_AXIS | SHOW_CURSOR | SHOW_VOLUME
		};
		...
		PlotStyle(ShowFlags flags, Color bg) :
			Style	{ bg },
			show	{ flags }
		{
		}
