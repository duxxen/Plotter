...
namespace Plotter
{
	...
	void Graph::graph(vector<Values> points)
	{
		this->points = points;
		for (auto p : points)
		{
	...
	void Graph::recompute()
	{
		...
		for (auto i = 0; i < points.size(); i++)
		{
			...
			gLines[i] = Vertex(coords, lColor);
			if (i % 2 == 0 && i < points.size() - 1)
			{
				vTriangles[i] = Vertex(coords, vColor);
				vTriangles[i + 1] = Vertex(Coords(coords.x, zero.y), vColor);
			}
		}
	}
	...
	void Graph::onStyleChanged()
	{
		...
		for (int i = 0; i < gLines.getSize(); i++)
			gLines[i].color = lColor;
		for (int i = 0; i < vTriangles.getSize(); i++)
